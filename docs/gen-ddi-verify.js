...
<h2>🔎 0) วิเคราะห์กรณี Midazolam + Morphine ที่ไม่ขึ้นเตือน — แก้แล้วใน v5.51.6</h2>
...
แต่โค้ดเดิมเอาตาราง <code>ddi_class_rules</code> จาก Supabase มา<b>แทนที่</b> (replace) ค่า default ทั้งชุด
— และตารางนั้น<b>มีข้อมูลไม่ครบ</b> (ขาด tag cnsDepress ของยากลุ่ม opioid/benzodiazepine)
ยาคู่นี้จึงหายไปจากชุดที่ใช้จริงบนหน้าเว็บ → ยืนยันได้ด้วย SQL ในหัวข้อ 5)<br><br>
<b>วิธีแก้ (ทำแล้ว, v5.51.6):</b> เปลี่ยนจาก "แทนที่" เป็น <b>"ผสานทับค่า default" (merge-over-defaults)</b> —
โค้ด default กลายเป็น<b>พื้นขั้นต่ำ (safety floor)</b> ที่ Supabase ลบทิ้งไม่ได้: class ของแต่ละยาจะ
<b>รวม (union)</b> ระหว่าง default กับ Supabase (admin เพิ่มได้ แต่ลบ tag ที่ vetted ในโค้ดออกจากหน้าจอไม่ได้)
ส่วนคู่ curated จะ code∪remote (admin แก้รายละเอียดคู่เดิมได้ ทับได้ แต่คู่ในโค้ดไม่หาย)
→ ตั้งแต่นี้ ตารางบน Supabase ที่ไม่ครบ/ล้าสมัย <b>จะบังอันตรกิริยาที่มีในโค้ดไม่ได้อีก</b>
