...
4. **ตัวส่งต่อต้องรู้ว่า "ถึงที่หมายแล้ว"** ← อาการรอบสอง: ปุ่มเปิด**ถูกหน้า**แล้ว แต่หน้านั้น
   **โหลดวนไม่หยุด** เพราะ LINE แปะ `?liff.state` กลับมาที่หน้าปลายทางอีก ตัวส่งต่อรุ่นแรก
   เทียบ URL ทั้งเส้น เห็นว่า "ไม่เหมือนเดิม" (ต่างกันแค่ `liff.state`) เลยส่งต่อซ้ำไม่รู้จบ
   → v5.83.1 เทียบแบบ **ตัดพารามิเตอร์ `liff.state` ออกก่อน** (path + query ที่เหลือ) —
   ต้องเทียบทั้ง path และ query ไม่ใช่ path อย่างเดียว ไม่งั้น deep link จากบอตที่ชี้
