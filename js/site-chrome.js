...
(function () {
  ...
  function build() {
    ...
    // Open/closed state persists across page navigations (sessionStorage), so the
    // rail does NOT disappear when you click through to another page. Default OPEN
    // on first entry; once hidden with ‹‹ it stays hidden as you navigate. Content
    // is pushed right (CSS body.sc-open) so the rail never overlaps it.
    function setOpen(o) {
    ...
    // swipe: right opens; left closes (when closed, only a swipe from the left
    // edge opens). NO auto-hide on scroll — the rail hides only via ‹‹ / swipe / Esc.
    ...
    // restore persisted state (default OPEN on first entry)
    var stored;
